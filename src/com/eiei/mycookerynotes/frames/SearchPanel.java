...
public class SearchPanel {
    private static JPanel contentPanel = ContentPanel.getContentPanel();
    ...
    public static void showResults(String query) {
        boolean match = false;
        ...
        if (!query.equals("")) {
            ...
            if (searchMode.equals(SearchTypes.BY_INGREDIENTS)) match = showIngredientsResults(query);
            if (!match) clearSearch();
            renewPanel(contentPanel);
        }
    }

    public static boolean showTitleResults(String query) {
    ...
    public static boolean showIngredientsResults(String query) {
        boolean matchFlag = false;
        for(int i = 0; i < MrChef.ReceiptList.size(); i++) {
            Dish matchDish = MrChef.ReceiptList.get(i);
   ...
   private static void addTextToMatchPane(JTextPane pane, String text, String option) {
        ...
        StylesForTextPanes.getBigHeader(doc);
